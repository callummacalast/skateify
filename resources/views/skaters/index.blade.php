<x-guest-layout>
    <section class="py-6  text-gray-100">
        <div class="container flex flex-col items-center justify-center mx-auto p-5">
            <div class="search w-full">
                <form action="{{ route('skater.search') }}" method="get">
                    ...
                    <div class="relative">
                        ...
                        <input type="search" id="default-search"
                            ...
                            placeholder="Search Mockups, Logos..." name="s"
                            value="{{ isset($search) ? $search : '' }}" required>
                        <button type="submit"
                            class="text-white absolute right-2.5 bottom-2.5 bg-blue-700 hover:bg-blue-800 focus:ring-4 focus:outline-none focus:ring-blue-300 font-medium rounded-lg text-sm px-4 py-2 bg-blue-600 hover:bg-blue-700 focus:ring-blue-800">Search</button>
                    </div>
                </form>
            </div>
            </h1>
            <div class="grid xl:grid-cols-4 lg:grid-cols-3 md:grid-cols-2 grid-cols-1 gap-5 w-full p-5">
                @foreach ($skaters as $skater)
                    <div
                        class="flex flex-col justify-between  p-6 shadow-md rounded-xl sm:px-12 bg-gray-900 text-gray-100">
                        ...
                        <div class="space-y-4 text-center divide-y divide-gray-700">
                            <div class="my-2 space-y-1">
                                <a href="{{ route('skater.show', $skater) }}">
                                    <h2 class="text-lg font-semibold sm:text-xl">{{ $skater->name }}</h2>

                                </a>
                                <p class="px-5 text-xs sm:text-base text-gray-400">
                                    {{ isset($skater->skater) ? $skater->skater->skill_level : '' }}</p>
                                <x-svgs.skateboard class="w-full h-5" />

                            </div>
                            <div class="flex justify-center pt-2 space-x-4 align-center">
                                <a rel="noopener noreferrer" href="#" aria-label="GitHub"
                                    class="p-2 rounded-md text-gray-100 hover:text-sky-400">
